...
public class TurtleRenderer extends Panel implements  DocumentListener {
	...
	private static final Dimension WINDOW_SIZE = new Dimension(900, 900);
	private static final int BOARD_SIZE = 100;
	private static Point START_LOCATION = new Point(0, 0);
	
	Pixel[][] board;
	final BoardMaker boardMaker;
	
	public TurtleRenderer() {
		boardMaker = new BoardMaker(BOARD_SIZE, START_LOCATION);
	...
	void changed(DocumentEvent de) {
		...
		} catch(PointOutOfBoardException pe) {
			JOptionPane.showMessageDialog(this, "Turtle overboard!!!");
			pe.printStackTrace();
		
		} catch (ProgramSyntaxException e){
			// nothing to do here, or you can print error message
